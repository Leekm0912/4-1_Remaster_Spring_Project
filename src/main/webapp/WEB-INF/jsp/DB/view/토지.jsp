...
<body>
	...
	if (type.equals("view")) {
		query = "select * from 상세매물_토지 order by 매물등록번호"; /* SQL 문 */
	} else if (type.equals("sell")) {
		query = "select * from 상세매물_토지 where 상세매물_토지.매물등록번호 not in (select 주문.매물_등록번호 from 주문) order by 상세매물_토지.매물등록번호";
	}else if (type.equals("update")) {
		query = "select * from 상세매물_토지 where 매도자명 = (select 매도자_이름 from 매도자 where id='" + id + "') order by 매물등록번호";
	}
	...
	<form action="to.jsp" method="post">
		...
		%>
			...
			for (int i = 0; i < row1.size(); i++) {
				...
				out.print("<td>" + formatter.format(row5.get(i)) + "평" + "</td>");
