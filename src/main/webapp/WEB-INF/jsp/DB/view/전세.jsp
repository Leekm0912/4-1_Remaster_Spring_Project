...
<body>
	...
	if (type.equals("view")) {
		...
		query = "select * from 상세매물_전세 where 상세매물_전세.매물등록번호 not in (select 주문.매물_등록번호 from 주문) order by 상세매물_전세.매물등록번호";
	} else if (type.equals("update")) {
		query = "select * from 상세매물_전세 where 매도자명 = (select 매도자_이름 from 매도자 where id='" + id + "') order by 매물등록번호";
	}
